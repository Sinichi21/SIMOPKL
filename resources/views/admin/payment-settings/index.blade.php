                ...
                <table class="table table-bordered" id="feeTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $setting->name }}</td>
                            <td>
                                {{ substr($setting->account_number, 0, 4) . '****' . substr($setting->account_number, -4) }}
                            </td>
                            <td>
                                @if ($setting->image_url)
                                    <img src="{{ asset('storage/' . $setting->image_url) }}" alt="Logo" width="50">
                                @else
                                    Tidak Ada
                                @endif
                            </td>
                            <td>
                                <div class="flex-row flex-wrap d-flex" style="gap: 0.5rem">
                                    <a href="{{ route('payment-settings.edit', $setting->id ) }}" class="w-100">
        ...
        $('.btn-delete').click(function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{route('payment-settings.delete')}}",
                        data: data,
                        headers: {
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            const table = $('#feeTable').DataTable({
                columnDefs: [{width: '5%', targets: 0}, {width: '10%', targets: 5}]
            });
        });
    </script>
    @endsection

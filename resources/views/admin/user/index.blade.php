                    ...
                    <div class="row no-gutters align-items-center">
                        <div class="mr-2 col">
                            <div class="mb-1 text-xs font-weight-bold text-success text-uppercase">
                                Awardee</div>
                            <div class="mb-0 text-gray-800 h5 font-weight-bold">{{ $totalAwardees }}</div>
                        </div>
                        <div class="col-auto">
        ...
        <div class="mb-4 col-xl-3 col-md-6">
            <div class="py-2 shadow card border-left-warning h-100">
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="mr-2 col">
                            <div class="mb-1 text-xs font-weight-bold text-info text-uppercase">Awardee & Admin
                            </div>
                            <div class="mb-0 text-gray-800 h5 font-weight-bold">{{ $totalAdminAwardees }}</div>
                        </div>
                        <div class="col-auto">
    ...
    <div class="mb-4 shadow card">
        <div class="card-body">
            ...
            <div class="mb-3 d-flex justify-content-end" style="gap: 0.5rem">
                ...
                <!-- Export Excel Button -->
                <div>
                    <button id="exportExcel" class="btn btn-primary" onclick="window.location.href='{{ route('user.export', ['kategori' => 'Admin', 'status' => 'Aktif']) }}'">
                        Export Excel
                    </button>
                </div>
                <div>
                ...
                <table class="table table-bordered" id="userTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $user->awardee->studyprogram->faculty->name}}</td>
                            <td>{{ $user->role_id == 2 ? 'Awardee' : 'Admin' }}</td>
                            <td>{{ $user->status == '1' ? 'Anggota Aktif' : 'Anggota Biasa' }}</td>
                            <td>
                                <div class="flex-row flex-wrap d-flex" style="gap: 0.5rem">
                                    <a href="{{route('user.show', ['user' => $user->id])}}" class="w-100">
                                        <button class="btn btn-primary btn-edit w-100">Detail</button>
                                    </a>
                                    <a href="{{route('user.edit', ['user' => $user->id])}}" class="w-100">
                                        <button class="btn btn-warning btn-edit w-100">Edit</button>
                                    </a>
                                    @if(!$user->admin)
                                    <button type="button" class="btn btn-info btn-status w-100"
                                        data-id="{{$user->id}}">{{$user->role_id == '3' ? 'Awardee' :
                                        'Admin'}}</button>
